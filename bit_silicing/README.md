...
Theory
A grayscale image is typically represented using 8 bits per pixel, where
Each pixel intensity ranges from 0 to 255.
Every bit position (from Least Significant Bit to Most Significant Bit) forms a bit plane.
Bit Plane Slicing
...
Extract all 8 bit planes (0th to 7th) using bitwise operations.
Display each extracted bit plane separately.
Remove selected bit planes (0th, 5th, and 7th) from the original image.
Display the reconstructed images after bit plane removal.
